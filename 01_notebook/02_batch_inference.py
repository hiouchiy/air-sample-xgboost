...
# MAGIC
# MAGIC > Prefer submitting from a terminal? The CLI equivalent is `02_cli/02_batch_inference.py` — run
...
# MAGIC Both single-GPU training (`01`) and the fan-out HPO (`02_cli/fanout_hpo.py`) set `@champion`
# MAGIC to the model they just trained, so **this step scores whichever you ran last**. To score a
# MAGIC specific model: re-run `01` or the fan-out (each re-promotes `@champion`), or set `MODEL_URI`
# MAGIC to a version/alias, e.g. `models:/<catalog>.<schema>.xgboost_classification/3` (empty
# MAGIC `MODEL_URI` = `@champion`).
